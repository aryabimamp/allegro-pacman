...
static void draw_power_speed(Map* M, const int row, const int col);
static ALLEGRO_BITMAP* Speed;
static ALLEGRO_BITMAP* Coin;
static ALLEGRO_BITMAP* Power;
...
Map* create_map(const char * filepath) {
    Speed = load_bitmap_resized("Assets/speed_cake.png", 20, 20);
    Power = load_bitmap_resized("Assets/star_power.png", 23, 23);
    Coin = load_bitmap_resized("Assets/Coin.png", 12 ,12);
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				case '#':
					...
					break;
				// [ TODO // DONE]
				// draw the power bean [ done ]
				case 'P':
					draw_power_beans(M, row, col, Power);
					break;
				case 'S':
					draw_power_speeds(M, row, col, Speed);
					break;
				case '.':
					draw_beans(M, row, col, Coin);
					break;
				default:
					break;
			}
...
void draw_beans(Map* M, const int row, const int col, ALLEGRO_BITMAP* Coin) {
    al_draw_bitmap(Coin, map_offset_x + col * block_width + block_width / 2.0 - 5, map_offset_y + row * block_height + block_height / 2.0 - 6, 0);
}

void draw_power_beans(Map* M, const int row, const int col, ALLEGRO_BITMAP* Speed) {
    al_draw_bitmap(Power, map_offset_x + col * block_width + block_width / 2.0 - 11, map_offset_y + row * block_height + block_height / 2.0 - 12, 0);
}

void draw_power_speeds(Map* M, const int row, const int col, ALLEGRO_BITMAP* Speed) {
    al_draw_bitmap(Speed, map_offset_x + col * block_width + block_width / 2.0 - 11, map_offset_y + row * block_height + block_height / 2.0 - 10, 0);
}
